...
use([BarChart, CanvasRenderer, PieChart, GridComponent, LabelLayout, TooltipComponent, LegendComponent]);
...
const topProvidersData = computed(() => {
  return useOrderBy(useToPairs(modelCounters.value.byProvider), (item) => item[1], 'desc')
    .slice(0, 5)
    .map(([name, value]) => ({
...
const topModelsData = computed(() => {
  return useOrderBy(useToPairs(modelCounters.value.byModel), (item) => item[1], 'desc')
    .slice(0, 5)
    .map(([name, value]) => ({
      name: `${name} (${value.toFixed(0)}%)`,
      value,
    ...
    useOrderBy(
      Array.from(groupedAvailability.value.entries()).filter(
        ([model, modelsMap]) => {
          ...
          const modelMatches =
            typeof model === 'string' && model.toLowerCase().includes(searchValue);
      ...
      [([model, _]) => modelCounters.value.byModel[model] || 0, ([_, modelsMap]) => modelsMap[0].upTimePercentage || 0,],
...
const setGroupedAvailability = async (type) => {
  ...
  'llama-3.1-405b-instruct-turbo': ['llama-3.1-405b-instruct-turbo', 'llama3.1-405b-instruct-turbo'],
  'llama-3.1-8b-instruct-turbo': ['llama-3.1-8b-instruct-turbo', 'llama3.1-8b-instruct-turbo'],
  'llama-3.1-lumimaid-70b': ['llama-3.1-lumimaid-70b', 'llama3.1-lumimaid-70b'],
  'llama-3.1-lumimaid-8b': ['llama-3.1-lumimaid-8b', 'llama3.1-lumimaid-8b'],
  'llama-3.1-nemotron-70b-instruct': ['llama-3.1-nemotron-70b-instruct', 'llama3.1-nemotron-70b-instruct'],
  'llama-3.2-11b-vision-instruct': ['llama-3.2-11b-vision-instruct', 'llama3.2-11b-vision-instruct'],
  ...
  'llama-3.2-3b-instruct-turbo': ['llama-3.2-3b-instruct-turbo', 'llama3.2-3b-instruct-turbo'],
  'llama-3.2-90b-vision-instruct': ['llama-3.2-90b-vision-instruct', 'llama3.2-90b-vision-instruct'],
  'llama-3.2-90b-vision-instruct-turbo': ['llama-3.2-90b-vision-instruct-turbo', 'llama3.2-90b-vision-instruct-turbo'],
  'llama-3.3-70b-instruct': ['llama-3.3-70b-instruct', 'llama3.3-70b-instruct'],
  'llama-3.3-70b-instruct-turbo': ['llama-3.3-70b-instruct-turbo', 'llama3.3-70b-instruct-turbo'],
  'llama-3-1-405b-instruct': ['llama-3-1-405b-instruct', 'llama3-1-405b-instruct'],
  'llama-3-1-70b-instruct': ['llama-3-1-70b-instruct', 'llama3-1-70b-instruct'],
  ...
  'llama-3-70b-instruct-lite': ['llama-3-70b-instruct-lite', 'llama3-70b-instruct-lite'],
  'llama-3-70b-instruct-turbo': ['llama-3-70b-instruct-turbo', 'llama3-70b-instruct-turbo'],
  ...
  'llama-3-8b-instruct-lite': ['llama-3-8b-instruct-lite', 'llama3-8b-instruct-lite'],
  'llama-3-8b-instruct-turbo': ['llama-3-8b-instruct-turbo', 'llama3-8b-instruct-turbo']
};
...
].map(keyword => keyword.toLowerCase()).sort((a, b) => b.length - a.length);
...
const sortedModelGroups = Object.entries(lowercaseModelGroups).sort(
  ...
  for (const row of modelsAvailabilityRef.value || []) {
    ...
    const matchedKeyword = partialKeywords.find((keyword) => groupKey.includes(keyword));
    if (matchedKeyword) {
      groupKey = matchedKeyword;
    }
    const matchedGroup = sortedModelGroups.find(([_, models]) => models.some((model) => model === groupKey || groupKey.includes(model)));
    if (matchedGroup) {
      const groupName = matchedGroup[0]; // Keep original casing
      const matchedVariant = matchedGroup[1].find(variant => modelName.includes(variant.toLowerCase()));
      ...
      if (matchedVariant) {
        ...
        if (matchIndex !== -1) {
          ...
          const suffix = matchIndex + variantLower.length < row.model.length ? 
  ...
  const modelRows = groupedRows.get(groupKey);
    const lastPreviousStatusDt = new Date(row.last_previous_status_dt || row.created_dt);

    if (!modelRows.has(providerModelKey)) {
      const model = models.value.get(`${row.provider}_${row.route}`);
      if (!model) {
        console.log(`Model ${row.provider}/${row.route} is not presented in models!`);
        continue;
      }
      ...
      const states = [
        ...
        ...Array(statusDots).fill(
          row.last_previous_status === 'AVAILABLE' ? 100 : row.last_previous_status === 'UNAVAILABLE' ? 0 : null,
        ),
      ];
      ...
      if (row.last_previous_status_dt && row.last_previous_status) {
        totalTracked = useMin([differenceInSeconds(nowDt, lastPreviousStatusDt), maxTotalTracked]);
    ...
    const secondsSinceStatus = useMin([differenceInSeconds(nowDt, statusDt), maxTotalTracked]);
    ...
    if (groupedRow.lastStatus && groupedRow.lastStatus.status !== row.status) {
      ...
      groupedRow.totalTracked = useMax([differenceInSeconds(nowDt, statusDt), groupedRow.totalTracked]);
    ...
    const statusDots = Math.floor(differenceInHours(nowDt, statusDt) / INTERVAL_HOURS) + 1;
      ...
      .map(([model, modelsMap]) => [
        ...
        useOrderBy(
          Array.from(modelsMap.values()).map((groupedRow) => {
            groupedRow.latenciesRating = groupedRow.latencies.map((latency, index) => {
              if (latency === null) return 0;
              if (bucketRanges[index].min === bucketRanges[index].max) return 100;
              return (
                100 - ((latency - bucketRanges[index].min) / (bucketRanges[index].max - bucketRanges[index].min)) * 100
              );
            });
            return groupedRow;
          }),
...
const triggerMailTo = () => {
  window.location = 'mailto:support@neuronicai.com?subject=Please add my aggregator to APIpie, here are the details';
};
...
<template>
  <div class="flex flex-col gap-5 w-full">
    ...
    <div class="grid gap grid-cols-1 xl:grid-cols-2 gap-5">
      <u-card class="md:h-[192px] xl:h-[368px]" :ui="{ body: { base: 'h-full' } }">
        <div class="flex flex-col md:flex-row gap-5 md:h-full">
          <charts-pie
            class="h-[190px] md:h-full"
            name="Top Providers"
            :data="topProvidersData"
            :legend-position="viewport.isGreaterOrEquals('xl') ? 'bottom' : 'right'"
          ></charts-pie>
          <div class="md:rotate-180 h-0.5 md:h-0 w-full md:w-[4px] dark:bg-gray-700"></div>
          <charts-pie
            class="h-[190px] md:h-full"
            name="Top Models"
            :data="topModelsData"
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-2 gap-4">
          <template v-for="(row, i) of statRows">
            <div class="flex flex-col gap-0.5">
              <div class="text-gray-400 text-sm font-medium">{{ row.name }}</div>
              <div class="text-2xl md:text-3xl font-semibold">{{ modelCounters.byType[row.type] }}</div>
            </div>
            <template v-if="viewport.isGreaterOrEquals('xl') || viewport.isLessThan('md')">
              <div
      ...
      <u-card class="xl:col-span-2">
        <div class="flex flex-col gap-5">
          ...
          <div class="flex flex-col gap-5">
            <template v-for="[groupKey, rows] in paginatedGroupedAvailability.entries()">
              <model-availability-card :model="{ ...rows[0], model: rows[0].model }" :group-key="groupKey">
              ...
              <div class="flex justify-between">
                ...
                <u-button
                  v-if="rows.length > 1"
                  size="2xs"
                  :icon="rows[0].isExpanded ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'"
                  color="white"
                  variant="solid"
                  trailing
